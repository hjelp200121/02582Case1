...
if __name__ == "__main__":
    ...
    MSE_CV_array = np.zeros((folds,models))
    ...
    for i in range(folds):
        ...
        MSE_CV_array[i, 0] = metrics.root_mean_squared_error(y_test, y_est1)

        y_est2 = LSTSQ_sklearn(X_train, y_train, X_test)
        MSE_CV_array[i, 1] = metrics.root_mean_squared_error(y_test, y_est2)

    # compute 2D cross validation error array as seen on slide 29 week 2
    MSE_CV_array = np.mean(MSE_CV_array, axis=0)
    print(MSE_CV_array)